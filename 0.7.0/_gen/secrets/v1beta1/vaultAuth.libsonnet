{
  ...
  spec: {
    ...
    appRole: {
      ...
      '#withSecretIDPath':: d.fn(help='"SecretIDPath is a file system path pointing to a file containing the plaintext Secret ID for the\\nAppRole Role to use for authenticating to Vault.\\nSecretIDPath and SecretRef are mutually exclusive, and only one should be specified."', args=[d.arg(name='secretIDPath', type=d.T.string)]),
      withSecretIDPath(secretIDPath): { spec+: { appRole+: { secretIDPath: secretIDPath } } },
      '#withSecretRef':: d.fn(help="\"SecretRef is the name of a Kubernetes secret in the consumer's (VDS/VSS/PKI) namespace which\\nprovides the AppRole Role's SecretID. The secret must have a key named `id` which holds the\\nAppRole Role's secretID.\\nSecretIDPath and SecretRef are mutually exclusive, and only one should be specified.\"", args=[d.arg(name='secretRef', type=d.T.string)]),
